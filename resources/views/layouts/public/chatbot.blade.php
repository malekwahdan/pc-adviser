...
{{-- <script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function addMessage(message, isUser = false) {
            ...
            // Important: Using innerHTML to properly render HTML links
            messageDiv.innerHTML = `<strong>${isUser ? 'You' : 'PC Assistant'}:</strong> ${message}`;
        ...
        async function sendMessage() {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    // The server now returns HTML with links - no need to replace newlines
                    // as the controller will handle proper formatting
                    addMessage(data.response);
                } else {
                    connectionStatus.textContent = 'Server responded with error: ' + response.status;
                    addMessage('Sorry, I encountered a problem communicating with the server. Please try again.');
        ...
        chatMessages.addEventListener('click', function(event) {
            // Find if a product link was clicked (either the <a> element or something inside it)
            let linkElement = event.target;

            // If clicked element is inside an <a> tag, find the parent <a> tag
            while (linkElement && linkElement !== chatMessages) {
                if (linkElement.tagName === 'A' && linkElement.classList.contains('product-link')) {
...
{{-- <script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        async function sendMessage() {
            ...
            try {
                ...
                if (response.ok) {
                    const data = await response.json();
                    // Format response text - replace newlines with <br> tags
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function sendMessage() {
                ...
                if (message) {
                    ...
                    userMessageElement.innerHTML = `<span>${message}</span>`;
                    ...
                    // Simulate response (replace with your actual chatbot logic)
                    setTimeout(function() {
                        let botResponse = '';

                        // Simple response logic (replace with your actual AI or backend integration)
                        if (message.toLowerCase().includes('gaming')) {
                            botResponse = "For gaming, I recommend a PC with at least an RTX 3060 or better GPU, Intel i5/Ryzen 5 or better CPU, 16GB RAM, and an SSD for storage. What's your budget range?";
                        } else if (message.toLowerCase().includes('budget') || message.toLowerCase().includes('office') || message.toLowerCase().includes('work')) {
                            botResponse = "For office work on a budget, I suggest a PC with an Intel i3/Ryzen 3 CPU, 8GB RAM, and an SSD. This setup will handle most productivity tasks smoothly without breaking the bank.";
                        } else if (message.toLowerCase().includes('video') || message.toLowerCase().includes('editing') || message.toLowerCase().includes('32gb')) {
                            botResponse = "For video editing, I recommend a workstation with an Intel i7/Ryzen 7 or better CPU, 32GB RAM, an RTX series GPU, and at least 1TB SSD. This will handle most video editing tasks efficiently.";
                        } else {
                            botResponse = "Thanks for your message! Could you tell me more about what you'll be using this PC for? This will help me give you the best recommendation.";
                        }

                        // Add bot response
                        const botMessageElement = document.createElement('div');
                        botMessageElement.className = 'bot-message';
                        botMessageElement.innerHTML = `<strong>PC Assistant:</strong> ${botResponse}`;
  ...
  if (event.target.classList.contains('product-link') ||
      (event.target.parentElement && event.target.parentElement.classList.contains('product-link'))) {

    // Prevent the default navigation behavior
    event.preventDefault();

    // Get the URL from the clicked link
    const url = event.target.href || event.target.parentElement.href;

    // Option 1: Open in a new tab
    window.open(url, '_blank');

    // OR Option 2: Navigate in the same window but keep chat state
    // window.location.href = url;

    // OR Option 3: Handle with custom behavior (e.g., show product in a modal)
    // showProductDetails(url);
  }
});
    </script> --}}
<style>
